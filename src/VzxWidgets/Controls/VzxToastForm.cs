using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Drawing;
using System.Drawing.Drawing2D;
using System.Windows.Forms;
using VzxWidgets.Helpers;

namespace VzxWidgets.Controls;

public enum ToastPosition
{
    BottomRight,
    TopRight,
    BottomLeft,
    TopLeft,
    BottomCenter,
    TopCenter
}

/// <summary>
/// Formulário flutuante individual e animado de Toast (Slide-in, Auto-dismiss com barra de tempo e Hover-pause).
/// </summary>
public class VzxToastForm : Form
{
    private readonly System.Windows.Forms.Timer _animTimer;
    private readonly System.Windows.Forms.Timer _dismissTimer;
    private readonly System.Windows.Forms.Timer _progressTimer;

    private ToastType _type = ToastType.Success;
    private string _title = "Notificação";
    private string _message = "Operação realizada com sucesso.";
    private Color _barColor = Color.FromArgb(48, 209, 88);
    private int _borderRadius = 10;
    private bool _closeHovered = false;

    private int _durationMs = 3500;
    private int _elapsedMs = 0;
    private bool _isClosing = false;
    private bool _isHovered = false;

    private int _targetX;
    private int _targetY;
    private int _startX;

    public event EventHandler? ToastClosed;

    public VzxToastForm(ToastType type, string title, string message, int durationMs = 3500)
    {
        _type = type;
        _title = title;
        _message = message;
        _durationMs = durationMs;

        _barColor = _type switch
        {
            ToastType.Success => Color.FromArgb(48, 209, 88),
            ToastType.Info => Color.FromArgb(155, 80, 255),
            ToastType.Warning => Color.FromArgb(255, 149, 0),
            ToastType.Error => Color.FromArgb(255, 69, 58),
            _ => Color.FromArgb(155, 80, 255)
        };

        FormBorderStyle = FormBorderStyle.None;
        StartPosition = FormStartPosition.Manual;
        ShowInTaskbar = false;
        TopMost = true;
        DoubleBuffered = true;
        Size = new Size(260, 64);
        BackColor = Color.FromArgb(18, 18, 24);
        Opacity = 0.0;

        SetStyle(ControlStyles.AllPaintingInWmPaint |
                 ControlStyles.OptimizedDoubleBuffer |
                 ControlStyles.ResizeRedraw |
                 ControlStyles.SupportsTransparentBackColor |
                 ControlStyles.UserPaint, true);

        // Timer de animação de posição e opacidade
        _animTimer = new System.Windows.Forms.Timer { Interval = 15 };
        _animTimer.Tick += AnimTimer_Tick;

        // Timer de progresso da barra
        _progressTimer = new System.Windows.Forms.Timer { Interval = 30 };
        _progressTimer.Tick += ProgressTimer_Tick;

        // Dismiss timer
        _dismissTimer = new System.Windows.Forms.Timer { Interval = durationMs };
        _dismissTimer.Tick += (s, e) =>
        {
            if (!_isHovered && !_isClosing)
            {
                CloseWithAnimation();
            }
        };
    }

    protected override bool ShowWithoutActivation => true;

    protected override CreateParams CreateParams
    {
        get
        {
            var cp = base.CreateParams;
            cp.ExStyle |= 0x08000000; // WS_EX_NOACTIVATE
            cp.ExStyle |= 0x00080000; // WS_EX_LAYERED
            return cp;
        }
    }

    public void ShowToast(int targetX, int targetY)
    {
        _targetX = targetX;
        _targetY = targetY;

        _startX = targetX + 40;
        Location = new Point(_startX, targetY);

        Show();
        _animTimer.Start();
        _progressTimer.Start();
        _dismissTimer.Start();
    }

    public void UpdateTargetPosition(int newX, int newY)
    {
        _targetX = newX;
        _targetY = newY;
    }

    private void AnimTimer_Tick(object? sender, EventArgs e)
    {
        if (!_isClosing)
        {
            if (Opacity < 0.98)
            {
                Opacity = Math.Min(1.0, Opacity + 0.12);
            }

            int dx = _targetX - Location.X;
            int dy = _targetY - Location.Y;

            if (Math.Abs(dx) > 1 || Math.Abs(dy) > 1)
            {
                Location = new Point(Location.X + (int)Math.Round(dx * 0.35), Location.Y + (int)Math.Round(dy * 0.35));
            }
            else
            {
                Location = new Point(_targetX, _targetY);
            }
        }
        else
        {
            Opacity = Math.Max(0.0, Opacity - 0.15);
            Location = new Point(Location.X + 6, Location.Y);

            if (Opacity <= 0.05)
            {
                _animTimer.Stop();
                _progressTimer.Stop();
                _dismissTimer.Stop();
                ToastClosed?.Invoke(this, EventArgs.Empty);
                Close();
                Dispose();
            }
        }
    }

    private void ProgressTimer_Tick(object? sender, EventArgs e)
    {
        if (!_isHovered)
        {
            _elapsedMs += _progressTimer.Interval;
            Invalidate();
        }
    }

    public void CloseWithAnimation()
    {
        if (_isClosing) return;
        _isClosing = true;
        _dismissTimer.Stop();
        _animTimer.Start();
    }

    protected override void OnMouseEnter(EventArgs e)
    {
        base.OnMouseEnter(e);
        _isHovered = true;
        Invalidate();
    }

    protected override void OnMouseLeave(EventArgs e)
    {
        base.OnMouseLeave(e);
        _isHovered = false;
        _closeHovered = false;
        Invalidate();
    }

    protected override void OnMouseMove(MouseEventArgs e)
    {
        base.OnMouseMove(e);
        bool inClose = e.X >= Width - 26 && e.Y <= 26;
        if (_closeHovered != inClose)
        {
            _closeHovered = inClose;
            Cursor = inClose ? Cursors.Hand : Cursors.Default;
            Invalidate();
        }
    }

    protected override void OnMouseClick(MouseEventArgs e)
    {
        base.OnMouseClick(e);
        if (e.X >= Width - 26 && e.Y <= 26)
        {
            CloseWithAnimation();
        }
    }

    protected override void OnPaint(PaintEventArgs pevent)
    {
        var g = pevent.Graphics;
        GraphicsHelper.ApplyHighQuality(g);

        var rect = new RectangleF(0, 0, Width - 1, Height - 1);
        using var path = GraphicsHelper.GetRoundedRectangle(rect, _borderRadius);
        using var brushBg = new SolidBrush(Color.FromArgb(20, 20, 27));
        using var penBorder = new Pen(Color.FromArgb(40, 40, 52), 1f);

        g.FillPath(brushBg, path);
        g.DrawPath(penBorder, path);

        var rectBar = new RectangleF(0, 6, 3.5f, Height - 12);
        using var brushBar = new SolidBrush(_barColor);
        g.FillRectangle(brushBar, rectBar);

        if (_durationMs > 0)
        {
            float progress = 1f - Math.Clamp((float)_elapsedMs / _durationMs, 0f, 1f);
            float barW = (Width - 10) * progress;
            if (barW > 0)
            {
                using var brushProg = new SolidBrush(Color.FromArgb(60, _barColor));
                g.FillRectangle(brushProg, 5, Height - 3, barW, 2);
            }
        }

        float iconX = 16;
        float iconY = Height / 2f - 2;
        using var penIcon = new Pen(_barColor, 2f)
        {
            StartCap = LineCap.Round,
            EndCap = LineCap.Round,
            LineJoin = LineJoin.Round
        };

        switch (_type)
        {
            case ToastType.Success:
                g.DrawLines(penIcon, new[] {
                    new PointF(iconX, iconY + 1),
                    new PointF(iconX + 3.5f, iconY + 4.5f),
                    new PointF(iconX + 9.5f, iconY - 3.5f)
                });
                break;

            case ToastType.Warning:
                g.DrawLine(penIcon, iconX + 4.5f, iconY - 6, iconX + 4.5f, iconY + 1);
                g.FillEllipse(brushBar, iconX + 3.5f, iconY + 4, 2.5f, 2.5f);
                break;

            case ToastType.Error:
                g.DrawLine(penIcon, iconX + 1, iconY - 4, iconX + 8, iconY + 4);
                g.DrawLine(penIcon, iconX + 8, iconY - 4, iconX + 1, iconY + 4);
                break;

            case ToastType.Info:
            default:
                g.FillEllipse(brushBar, iconX + 3.5f, iconY - 6, 3, 3);
                g.DrawLine(penIcon, iconX + 5f, iconY - 1, iconX + 5f, iconY + 6);
                break;
        }

        using var titleFont = new Font("Segoe UI Semibold", 9.5f, FontStyle.Bold);
        TextRenderer.DrawText(g, _title, titleFont, new Rectangle(36, 12, Width - 64, 20), Color.White,
            TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);

        using var msgFont = new Font("Segoe UI", 8.5f);
        TextRenderer.DrawText(g, _message, msgFont, new Rectangle(36, 32, Width - 64, 20), Color.FromArgb(170, 170, 188),
            TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);

        Color xColor = _closeHovered ? Color.White : Color.FromArgb(120, 120, 140);
        using var penX = new Pen(xColor, 1.3f) { StartCap = LineCap.Round, EndCap = LineCap.Round };
        float cx = Width - 15;
        float cy = 15;
        g.DrawLine(penX, cx - 3.5f, cy - 3.5f, cx + 3.5f, cy + 3.5f);
        g.DrawLine(penX, cx + 3.5f, cy - 3.5f, cx - 3.5f, cy + 3.5f);
    }
}
